...
export class VisualizarFotosPage {
  ...
  comentar(obj){
    let chave=obj.payload.key;
    let documento=obj.payload.val();

  //se tiver adiciona se nao cria 
if (documento.comentarios){
  obj.comentarios.push(novoComentario);
}
else{
  obj.comentarios=[novoCOmentario];
}
   this.fotosProvider.update(chave,documento);  
